...
case class ConnectionReadProperties(url: String, user: String, password: String, tableName: String, partitionNum: Int)
...
class PostgresScanBuilder(options: CaseInsensitiveStringMap) extends ScanBuilder {
  override def build(): Scan = new PostgresScan(ConnectionReadProperties(
    options.get("url"), options.get("user"), options.get("password"), options.get("tableName"), Integer.parseInt(options.get("partitionNum"))
  ))
}
